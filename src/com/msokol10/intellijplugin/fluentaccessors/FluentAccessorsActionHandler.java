...
public class FluentAccessorsActionHandler extends EditorWriteActionHandler {
    ...
    private void doExecuteAction(final Project project, final PsiClass psiClass, final Editor editor) {
        ...
        if (candidateFields.size() == 0) {
            HintManager.getInstance().showErrorHint(editor, "No members to generate fluent accessors have been found.");
            return;
        }

        List<PsiFieldMember> psiFieldMembers = new LinkedList<>();
    ...
    private List<PsiField> getCandidateFields(final PsiClass psiClass) {
        ...
        List<PsiField> candidateFields = new LinkedList<>();
    ...
    private Collection<PsiField> filterExistCandidates(final PsiClass psiClass, final List<PsiField> candidateFields) {
        Collection<PsiField> result = new LinkedList<>();
    ...
    private void chooseMemberAndRun(final Project project,
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            FluentAccessorsMemberChooser chooser
                    = new FluentAccessorsMemberChooser(classMembers, project);
            chooser.setTitle("Generate Fluent Accessors");
